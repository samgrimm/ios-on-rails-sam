describe 'GET /v1/events/nearests?lat=&lon=&radius=' do
	it 'returns the events closest to the lat and the lon' do 
		near_event = FactoryGirl.create(:event, lat: 37.760322, lon: -122.429667)
		farther_event = FactoryGirl.create(:event, lat: 37.760321, lon: -122.429667)
		FactoryGirl.create(:event, lat: 37.687737, lon: -122.470608)
		lat = 37.771098
    	lon = -122.430782
    	radius = 5
	...
	it 'returns an error message when no event is found' do
    	...
    	expect(JSON.parse(response.body)).to eq({ 'message' => 'No Events Found' })
